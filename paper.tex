...
coefficients are given. For each blade element, relative flow velocity
$\vec{U}_\mathrm{rel}$ and angle of attack $\alpha$ are computed by adding the
vectors of relative blade motion $-\omega r$, where $\omega$ is the rotor
angular velocity and $r$ is the blade element radius, and the local inflow
velocity $\vec{U}_\mathrm{in}$, a diagram of which is shown in
